...
테스트 코드를 작성 했다. 개별적으로 작동할 경우엔 모두 잘 작동하지만, 클래스 레벨에서 전체 테스트를 작동시킬 경우에는 `findByName`의 테스트가 실패한다.

이는 테스트의 순서 보장이 되지 않는 문제 때문에 발생한다. 즉, 테스트마다 `repository` 내부의 `store`에 저자


테스트 순서보장이 되지 않는 문제 -> 테스트마다 메모리에 저장되는 객체가 달라서, 다른 테스트에 영향이 간다. -> 테스트가 끝날 때 마다 저장소를 클리어한다. -> `@AfterEach` 어노테이션을 적용한 함수는 각 테스트 종료 시점에 작동한다.
