...
`정적 컨텐츠 방식`은 말 그대로 동적으로 변화하지 않는, 서버 내에 저장되어 있는 파일을 그대로 클라이언트에게 전달 하는 것이다.

`MVC와 템플릿 엔진`을 통한 전달 방식은 클라이언트에게 보여줄 유저 인터페이스(UI)와 데이터를 동시에 전달하는 방식이다. 특정 조건에 의해 동적으로 유저 인터페이스나 데이터가 변화하
