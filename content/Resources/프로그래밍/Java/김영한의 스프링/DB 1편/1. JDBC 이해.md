...
`DriverManager.getConnection(..)`를 통해 드라이버를 찾고, 해당 드라이버를 통해 H2 데이터베이스 커넥션을 획득했다. 그런데 `DriverManager`는 어떻게 이 과정을 처리하는 것일까?

해답은 `URL`에 있다. 다시 돌아가서 `ConnectionConst`의 `URL` 상수를 살펴보자.
...
`DriverManager.getConnection(URL, USERNAME, PASSWORD)`를 호출하면 

![[Pasted image 20241211193742.png]]
