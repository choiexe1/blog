...
`DriverManager.getConnection(URL, USERNAME, PASSWORD)`를 호출하면 `DriverManager`는 드라이버들에게 순서대로 입력된 파라미터 정보를 넘겨서 커넥션을 획득 할 수 있는지 확인한다.

![[jdbc-2.png]]

이제 `URL, USERNAME, PASSWORD` 정보를 전달 받은 드라이버들은 다음과 같은 절차를 진행한다.
1. `URL` 정보를 체크해서 본인이 처리할 수 있는 요청인지 확인한다. 예를 들어서 `URL`이 `jdbc:h2`로 시작하면 이것은 H2 데이터베이스에 접근하기 위한 규칙이다. 따라


다시 돌아가서 `ConnectionConst`의 `URL` 상수를 살펴보자.
