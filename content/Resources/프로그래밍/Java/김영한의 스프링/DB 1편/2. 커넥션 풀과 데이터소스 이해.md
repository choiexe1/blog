...
커넥션 풀은 기본적으로 사용 후 반납하여 재사용하는 개념이므로 11번째 커넥션 획득 요청은 다음의 로그처럼 대기 상태,  `wating`인 상태로 대기하게 된다.
...
> ⏳`connectionTimeout`
> 
> This property controls the maximum number of milliseconds that a client (that's you) will wait for a connection from the pool. If this time is exceeded without a connection becoming available, a SQLException will be thrown. Lowest acceptable connection timeout is 250 ms. _Default: 30000 (30 seconds)_
> 
