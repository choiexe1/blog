...
출력 결과는 이전의 결과와 동일하지만 시간 측정 로직이 빠지고 나니 훨씬 깔끔해졌다. **템플릿 메서드 패턴은 이렇게 다형성을 사용해서 변하는 부분과 변하지 않는 부분을 분리하는 방법이다.** 자바의 [[추상 클래스와 추상 메서드]]는 구현된 메서드와 추상 메서드를 동시에 제공할 수 있다. 이를 이용한 것이다.
