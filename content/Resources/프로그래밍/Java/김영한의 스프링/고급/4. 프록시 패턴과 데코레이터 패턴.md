...
프록시 객체는 `OrderxxxV1`을 구현하므로 다형성에 의해서 `OrderxxxV1`으로 반환할 수 있다. **프록시를 실제 객체 대신 등록** 하는 것이다. 몇 번이나 반복해서 말하는 거지만, 프록

의존관계가 조금 복잡해지긴 하지만 실제 호출 객체에선 핵심 로직만 들어있게 되므로 아주 깔끔하게 작성할 수 있다.
