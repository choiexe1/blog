...
```
- `Class.forName("hello.proxy.jdkdynamic.ReflectionTest$Hello")`: 클래스 메타정보를 획득한다. 참고로 내부 클래스는 구분을 위해 `$` 기호를 사용한다. `ReflectionTest` 클래스에 정적 중첩 클래스 `Hello`가 존재하기 때문에 `$`를 사용한다. [[중첩 클래스와 내부 클래스#정적 중첩 클래스 (Static Nested Class)]]에서 정적 중첩 클래스를 
