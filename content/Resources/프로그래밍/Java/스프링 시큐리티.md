...
구성된 대로 Spring Security는 해당 요청을 가로채서 사용자를 인증하는 필터를 제공합니다. 사용자가 인증에 실패하면 페이지가 /login?error로 리디렉션되고 페이지에 적절한 오류 메시지가 표시됩니다. 로그아웃에 성공하면 /login?logout으로 전송되고 페이지에 적절한 성공 메시지가 표시됩니다. 마지막으로 방문자에게 현재 사용자 이름을 표시하고 로그아웃할 수 있는 방법을 제공해야 합니다. 이렇게 하려면 다음 목록(src/main/resources/templates/hello.html에서 가져온)과 같이 현재 사용자와 인사하고 로그아웃 양식을 포함하도록 hello.html을 업데이트합니다:
