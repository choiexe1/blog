...
여기서 `current transaction`은 외부 트랜잭션을 의미한다. 외부 트랜잭션을 일시 중지하고, 새로운 트랜잭션을 시작하는 것이다.

트랜잭션 매니저가 동작하는 것을 다시 복기해보면 트랜잭션이 신규 트랜잭션인지 아닌지에 따라 동작이 달라진다. 따라서 내부 트랜잭션 `inner`는 `isNewTransaction()`이 `true`이기 때문에 롤백시 물리 트랜잭션이 롤백되고 커밋시 물리 트랜잭션이 커밋된다.

완전히 독립적인 커넥션으로 트랜잭션을 수행하는 것이다.
