...
먼저 출력된 로그를 한 줄씩 천천히 살펴보면 `Participating in existing transaction`라는 로그가 있다. 기존 트랜잭션에 참여한다는 로그다.

그 이후에 내부 트랜잭션을 커밋했는데, 커밋되지 않았음을 알 수 있다. 실제로 내부 트랜잭션에서는 커밋하지 ㅇ
