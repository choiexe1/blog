...
스프링은 트랜잭션을 시작한 외부 트랜잭션이 물리 트랜잭션을 관리하도록 하므로 이 경우 `commit()`을 호출하면 물리 커밋 된다. (실제 데이터베이스에 커밋)

그러나 해당 트랜잭션에 참여하는 내부 트랜잭션은 신규 트랜잭션이 아니므로 `isNewTransaction()`이 `false`다. 
