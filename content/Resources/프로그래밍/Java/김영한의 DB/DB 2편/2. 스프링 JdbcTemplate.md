...
먼저 `JdbcTemplate`를 사용하려면 `JdbcTemplate`에 데이터 소스를 주입 해야한다. 당연한 소리지만 데이터소스를 기반으로 해당 데이터베이스에 대한 커넥션을 획득해서 사용해야 하기 때문이다.

위의 코드와 같이 `DataSource`가 필요하다. 이 데이터소스는 수동으로 직접 주입할 수도 있지만 스프링 부트가 자동으로 등록해준 데이터소스를 사용한다.
