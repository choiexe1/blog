...
먼저 `JdbcTemplate`를 사용하려면 위의 코드와 같이 `DataSource`가 필요하다. 이 데이터소스는 수동으로 직접 주입할 수도 있지만 스프링 부트가 자동으로 등록해준 데이터소스를 사용한다.

스프링 컨테이너가 스프링 빈으로 등록한 데이터소스는 기본적으로 `HikariCP`를 사용한다. 따라서 HikariDataSource
