...
- 컨트롤러와 서비스에서 의존하던 `SQLException`을 리포지토리 계층에서 캐치하고 언체크 예외로 변경한 덕에 더이상 의존하지 않게 됐다.
- 컨트롤러와 서비스 입장에서는 어짜피 처리할 수 없는 예외이므로, 무시하게 되고 예외는 계속 상위 계층으로 던져진다.
...
- 최종적으로 서블릿이나 `ExceptionHandler`가 해당 예외를 처리하게 된다.
