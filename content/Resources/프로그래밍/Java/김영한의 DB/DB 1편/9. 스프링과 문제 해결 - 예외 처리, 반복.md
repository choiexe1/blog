...
> [!error]  그런데 `SQLException`에 들어있는 오류 코드를 활용하기 위해서 `SQLException`을 서비스 계층으로 던지게 되면 서비스 계층이 `SQLException`이라는 `JDBC` 기술에 의존하게 되면서 지금까지 우리가 개선했던 서비스 계층의 순수성이 무너진다.
