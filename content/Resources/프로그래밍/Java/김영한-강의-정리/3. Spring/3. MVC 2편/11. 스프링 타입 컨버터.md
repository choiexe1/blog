...
`${{number}}`: 뷰 템플릿은 데이터를 문자로 출력한다. 따라서 컨버터를 적용하게 되면 `Integer` 타입에서 `String`으로 변환하는 타입 컨버터를 사용하는 것이다. 여기서는 `IntegerToStringConverter`가 사용되었다. 그런데 타임리프는 기본적으로 숫자를 문자로 자동 변환하기 때문에 컨버터를 적용할 ㄸ
