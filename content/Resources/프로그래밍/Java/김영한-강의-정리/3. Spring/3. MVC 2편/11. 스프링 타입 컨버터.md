...
테스트가 잘 작동한다. 그리고 이 컨버터 자체가 너무 재미있는 기능이다.

아무튼 타입 컨버터 인터페이스는 단순해서 이해하기 쉽다. 그런데 이렇게 타입 컨버터를 하나하나 직접 사용하면 개발자가 직접 컨버팅 하는 것과 큰 차이가 없다. 몇 줄의 코드를 더 작성해야 하는건 마찬가지다.
