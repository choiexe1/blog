...
이해를 돕기위해, 앞서 학습했던 [[8. HTTP 메세지 컨버터]]를 살펴보면 다음과 같은 API 요청을 제어하는 컨트롤러의 경우에는 기본적으로 `HttpMessageConverter`가 적용된다. 그리고 `HttpMessageConverter`는 내부적으로 어떤 라이브러리를 사용하고, 해당 라이브러리에서 결과물이 결정되는 것이다.
...
Links to this page: [[의존관계 자동 주입]], [[8. HTTP 메세지 컨버터]]