...
## FilterChainProxy
스프링 시큐리티는 프록시 패턴을 사용하여 `DelegatingFilterProxy`를 필터 체인에 등록한다.

그리고 `DelegatingFilterProxy`는 내부에 `FilterChainProxy`를 가지고 있다. 이는 스프링 빈이기 때문에 일반적으로 `DelegatingFilterProxy`로 래핑된다.

이해하기가 어려운데, 단순하게 생각해보면 서블릿 필터의 `doFilter()`를 통해 요청을 내부적으로 처리할 수 있도록 래핑되어 있는 것 같다.

이것은 마치 프론트 컨트롤러 패턴과 유사하다.
