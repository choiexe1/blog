...
- `api/messages`에 요청
- `/api/`의 `SecurityFilterChain0` 패턴에서 먼저 일치하므로 SecurityFilterChainn에서도 일치하더라도 SecurityFilterChain0만 호출됩니다. 메시지의 URL이 요청되면 /api/**의 SecurityFilterChain0 패턴에서 일치하지 않으므로 FilterChainProxy는 각 SecurityFilterChain을 계속 시도합니다. 일치하는 보안 필터 인스턴스가 없다고 가정하면 SecurityFilterChainn이 호출됩니다. 보안 필터 인스턴스가 세 개만 구성되어 있는 것을 확인할 수 있습니다. 그러나 SecurityFilterChainn에는 4개의 보안 필터 인스턴스가 구성되어 있습니다. 각 SecurityFilterChain은 고유할 수 있으며 개별적으로 구성할 수 있다는 점에 유의해야 합니다. 실제로 애플리케이션에서 Spring Security가 특정 요청을 무시하도록 하려는 경우 SecurityFilterChain에는 보안 필터 인스턴스가 0개가 있을 수 있습니다.
