...
```
- 인터셉터는 `HttpServletRequest`, `HttpServletResponse`를 파라미터로 제공한다.
- 또 메서드마다 다른 파라미터들도 제공하는데, `HandlerMethod` 파라미터와 `ModelAndView`, 그리고 예외가 담겨있는 `ex` 파라미터다.
- `HandlerMethod`엔 이 핸들러가 무엇을 호출하는지에 대한 다양한 정보가 들어있다.
- `ModelAndView`에는 뷰와 모델이 담겨있다.
...
- `addPathPatterns()`는 인터셉터가 적용 될 URL 패턴이다.
- `excludePathPatterns()`는 그 중 제외 될 URL 패턴이다.
