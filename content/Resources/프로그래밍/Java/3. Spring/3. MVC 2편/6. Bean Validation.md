...
따로 주의할 점은 `@ModelAttribute("item")`의 작동 방식이다. `"item"`을 생략하면 템플릿 엔진에서 렌더링 될 때 모델의 키 값이 `itemSaveForm`이 되어서 템플릿 파일에서 `th:object` 속성의 값도 모두 변경해야 하기 때문이다. 이 내용은 앞서 [[7. 스프링 MVC - 기본 기능#HTTP 요청 파라미터 - @ModelAttribute|@ModelAttribute]]에서 학습했다.

## Bean Validation - HTTP 메시지 컨버터
`@Valid`, `@Validated`는 `HttpMessageConverter`에도 적용할 수 있다. `HttpMessageConverter`에 대한 내용은 [[8. HTTP 메세지 컨버터|HTTP 메세지 컨버터]] 문서에 정리되어 있다.
...
Links to this page: [[7. 스프링 MVC - 기본 기능]], [[8. HTTP 메세지 컨버터]]