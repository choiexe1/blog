...
따로 주의할 점은 `@ModelAttribute("item")`의 작동 방식이다. `"item"`을 생략하면 템플릿 엔진에서 렌더링 될 때 모델의 키 값이 `itemSaveForm`이 되기 때문이다. 앞서 [[7. 스프링 MVC - 기본 기능#HTTP 요청 파라미터 - @ModelAttribute|@ModelAttribute]]
