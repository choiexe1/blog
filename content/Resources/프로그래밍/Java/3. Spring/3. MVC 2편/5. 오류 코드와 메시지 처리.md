...
`init` 메서드는 이 컨트롤러가 호출이 될 때마다 항상 새로운 `WebDataBinder`를 내부적으로 만들고 생성된 `WebDataBinder`에 검증기를 추가한다. 이렇게 `WebDataBinder`에 검증기를 추가하면 해당 컨트롤러에서는 검증기를 자동으로 적용할 수 있다. 
