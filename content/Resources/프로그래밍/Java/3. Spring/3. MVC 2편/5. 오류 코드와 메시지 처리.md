...
- **validate(Object target, Errors errors)**: 실질적으로 검증 작업을 수행하는 메서드다. 첫 번째 매개변수인 `Object`는 검증할 대상인 객체이고, `Errors`는 `BindingResult`의 부모 클래스이다. 따라서 다형성에 의해 `BindingResult`를 입력할 수 있다.
