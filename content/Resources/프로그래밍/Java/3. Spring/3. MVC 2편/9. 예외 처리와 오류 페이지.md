...
WAS(sendError 호출 기록 확인) <-
이 등록 과정을 마치고 서블릿 컨테이너에 에러가 전달되면 등록했던 `ErrorPage` 객체를 통해서 지정된 상태 코드를 처리하는 컨트롤러를 서버 내부적으로 다시 요청한다.

말 그대로, 서버 내부적으로 해당 컨트롤러를 다시 호출하는데 조금 생각해보면 문제가 있음을 알 수 있다.
