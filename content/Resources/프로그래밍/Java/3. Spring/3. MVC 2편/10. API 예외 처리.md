...
3. 인터셉터의 `preHandle`을 호출한다.
4. `preHandle` 호출이 끝나면 핸들러 어댑터를 이용해 핸들러를 호출한다.
5. 인터셉터에서 예외 발생 시 `postHandle`은 호출되지 않는다. 다음으로 `afterCompletion`이 호출되고 이후에 `WAS`에 예외가 전달된다.

스프링 인터셉터의 동작 흐름은 [[8. 로그인 처리 2 - 필터와 인터셉터#]]

#### ExceptionResolver 적용 후
![[exception-resolver-2.png]]
`ExceptionResolver`를 적용하게 되면 
