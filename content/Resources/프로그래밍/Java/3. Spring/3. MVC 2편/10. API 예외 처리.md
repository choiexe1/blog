...
`@ExceptionHandler`는 컨트롤러 스코프로 작동한다. 따라서 다른 컨트롤러와는 관계가 없다. 그런데 만약 `UserException`을 처리하는 예외 핸들러 외에 개발자가 명시하지 못한 다른 예외가 발생할땐 어떻게 해야 할까?

다음과 같이 `Exception`을 직접 사용하면, 런타임에 발생하는 오류는 모두 이 예외 핸들러에서 처리할 수 있다. 
...
Links to this page: [[7. 스프링 MVC - 기본 기능]], [[8. 로그인 처리 2 - 필터와 인터셉터]], [[9. 예외 처리와 오류 페이지]]