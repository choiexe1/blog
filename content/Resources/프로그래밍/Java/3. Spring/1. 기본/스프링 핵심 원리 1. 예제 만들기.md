...
## 객체 지향 원리 적용
서비스 오픈 직전에 할인 정책을 현재 존재하는 `FixDiscountPolicy`(정액할인제)가 아닌 `RateDiscountPolicy`(정률할인제)로 변경하게 되는 시나리오를 가정한다.
...
구현은 끝났으므로, 테스트 코드를 작성한다. 인텔리제이에서 메서드 이름에 포인터를 두고 `CMD+SHIFT+T`를 하면 테스트를 자동으로 생성해준다.
...
```
- `@DisplayName()`: 이 어노테이션은 테스트 시 출력되는 이름을 변경해준다.
