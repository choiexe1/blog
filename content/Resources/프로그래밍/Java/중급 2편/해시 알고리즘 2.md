...
예를 들어서 문자 `A`는 `65`, `B`는 `66`으로 표현된다. 가장 단순하게 `char`형을 `int`형으로 캐스팅하면 문자의 고유한 숫자를 확인할 수 있다.

그리고 `AB`와 같이 연속된 문자는 각각의 문자를 더하는 방식으로 숫자를 표현하면 된다. `65`와 `66`을 더하면 `131`이 된다.
